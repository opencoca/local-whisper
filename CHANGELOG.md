...
- App icon swapped to the Sage.is brand mark: hex constellation S with a microphone glyph below, on a macOS-style squircle background. The editable composite source (SVG + `sage-hex.png` + `sf-mic.png`) lives in `Talking/Resources/AppIcon-source/` along with a `build-icon.sh` script that re-renders `AppIcon.icns` from the SVG.
